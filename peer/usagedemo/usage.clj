;;assuming we're running interactively from
;;~/usagedemo, and that we have our files
;;at ~/usagedemo/usage.clj, m4_book_AP.xlsx
(ns taa.usage
  (:require [taa [core :as core]
                 [capacity :as capacity]]))

;;A bunch of these entries are for other pieces of the
;;pipeline, currently included for expediency.
;;We can trim them down in the future.

;;This is the root path to our project.
;;tbd, change to resolve res.
(def inputs-outputs-path (io/file-path "~/usage-demo"))
...
(def input-map-AP
  {:resources-root inputs-outputs-path
   ...
   :merge-rc? true
   ;;this kicks us over to hazelcast.
   :run-site :cluster})

(defn run-it []
  (core/taa-runs
   (io/file-path "~/usage-demo/m4_book_AP.xlsx")
   input-map-AP))
